-- TDS Farmer V2 - Money Optimized Edition
...
MainStroke.Color = Color3.fromRGB(139, 0, 255) -- Фиолетовый стиль TDS
...
-- === ОБНОВЛЕННЫЙ ВЕБХУК (MONEY EDITION) ===
function _E.sendMoneyWebhook(currentCoins, totalReceived)
    local webhook = getgenv().Webhook
    if not webhook or webhook == "" then
        _G.print("Webhook URL is missing", "error")
        return false
    end
    ...
    task.spawn(function()
        ...
        if success and (response.StatusCode == 200 or response.StatusCode == 204) then
            ...
            _G.print("Webhook failed to send", "error")
        end
    end)
end
...
SaveBtn.MouseButton1Click:Connect(function()
    ...
    _G.print("Settings updated and saved to " .. CONFIG_FILE, "success")
end)
...
-- Экспорт глобальных функций для других скриптов
_G.sendWebhook = _E.sendMoneyWebhook

-- === ФИНАЛЬНЫЙ ВЫВОД (ИСПРАВЛЕНО) ===
_G.print("TDS Farmer V2 Started", "system")
_G.print("your reward: " .. tostring(getgenv().RewarmA or 0), "debug")
